...
export interface AnsibleLayerModel {
  currentBom: BillOfMaterialModel
  boms: BillOfMaterialModel[]
  inputs: AnsibleInputModel[]
  dependencies: AnsibleDependencyModel[]
}

export interface AnsibleInputModel {
  name: string
  dependency: AnsibleDependencyModel
  output: string
}

export interface AnsibleDependencyModel {
  name: string
  path: string
  outputs: string[]
}

export class AnsibleLayer implements AnsibleLayerModel, OutputFile {
  name: string = 'ansible.yaml';
  ...
  processDependencies() {
    ...
    const addInput = (input: AnsibleInputModel): AnsibleInputModel => {
    ...
    needs
      .forEach((layerNeed: LayerNeeds) => {
        ...
        if (need === 'cluster') {
          ...
          const dependency: AnsibleDependencyModel = {
            name: 'cluster',
...
// TODO: Investigate changing from Terragrunt to Ansible
${this.inputBlock()}
`)
  }
...
export interface AnsibleBaseModel {
}

// TODO: Change to Ansible
export class AnsibleBase implements AnsibleBaseModel, OutputFile {
  name: string = 'ansible.yaml';
  type: OutputFileType = OutputFileType.terraform;
